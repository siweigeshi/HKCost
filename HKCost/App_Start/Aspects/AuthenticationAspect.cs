...
namespace HKCost.App_Start.Aspects
{
    ...
    public class AuthenticationAspect
    {
        ...
        public bool IsAuthenticationByApi(HttpControllerContext context, out string resultStr)
        {
            ...
            //#region 检测url 防止sql注入
            //op = Tools.JudgeSqlInjection.JudgeByUrl(context);
            //if (!op)
            //{
            //    resultStr = "sqlInjection";
            //    return op;
            //}
            //#endregion
            ...
            #region 判断用户是否登陆
            //判断用户是否登陆
            //if (string.IsNullOrEmpty(Common.CommonClass.GetCookie.GetCurUser().userId)) //判断用户是否登陆
            //{
            //    resultStr = "noLogin";
            //    op = false;
            //}
            ...
            else //判断url权限
            {
                if (controller.ToLower() != "base")//排除controller为BaseController
                {
                    ...
                    //Common.CommonClass.CookieCurUser CurUser = Common.CommonClass.GetCookie.GetCurUser();//获取Cookie信息
                    //Console.Write(CurUser.userId + "," + CurUser.name + "," + CurUser.defaultOrgId);
                    //userId = CurUser.userId;
                    //orgId = CurUser.defaultOrgId;
                    Base_UserInfo user = Tools.SessionHelper.GetSession<Base_UserInfo>(Tools.SessionHelper.SessinoName.CurUser);
                    //判断用户权限
                    if (!IsAllowByActionUrl(user.OID, user.Orgs.FirstOrDefault().OID, url))
                    {
                        resultStr = "noAuth";
                        op = false;
                    }
