 ...
 * @private
 */
Ext.define('ExtFrame.view.extEncap.plugin.ViewDropZone', {
    ...
    getPosition: function (e, node) {
        ...
        if (this.appendOnly) {
            if (noAppend) {
                return this.isAppentToLeafNode ? 'append' : false;
            } else {
                return 'append';
            }
        }
        ...
        var num = 3;
        if (noAppend) {
            num = this.isAppentToLeafNode && noAppend ? 3 : 2;
        }
        fragment = (region.bottom - region.top) / num;
        if (y >= region.top && y < (region.top + fragment)) {
            return 'before';
        }
        else if (!noBelow && (!(num == 3 && this.isAppentToLeafNode) || (y >= (region.bottom - fragment) && y <= region.bottom))) {
            return 'after';
        }
        else {
            return 'append';
